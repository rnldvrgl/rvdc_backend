...
# All times are in UTC (Philippines Time is UTC+8 - logs show PHT via TZ=Asia/Manila)
# ============================================================================

TZ=Asia/Manila

# DAILY TASKS (UTC times - Philippines Time = UTC+8)
# 1:00 PM UTC = 9:00 PM PHT - Auto-close open attendance records
0 13 * * * /opt/cron-scripts/auto-close-attendance.sh

# 11:00 PM UTC = 7:00 AM PHT (+1 day) - Send clock-in reminder
0 23 * * * /opt/cron-scripts/clock-in-reminder.sh

# 10:00 AM UTC = 6:00 PM PHT - Send clock-out reminder (1st group)
...
# 6:00 PM UTC = 2:00 AM PHT (+1 day) - Delete old notifications (older than 7 days)
0 18 * * * /opt/cron-scripts/delete-old-notifications.sh

# 7:00 PM UTC = 3:00 AM PHT (+1 day) - Cleanup archived quotations (older than 14 days)
0 19 * * * /opt/cron-scripts/cleanup-archived-quotations.sh

# WEEKLY TASKS (UTC times)
# 3:00 PM UTC Friday = 11:00 PM PHT Friday - Fix last week's attendance & refresh payroll
0 15 * * 5 /opt/cron-scripts/weekly-attendance-payroll-fix.sh

# YEARLY TASKS (UTC times with date boundaries)
# 6:00 PM UTC December 31 = 2:00 AM PHT January 1 - Update holiday years
0 18 31 12 * /opt/cron-scripts/yearly-update-holidays.sh

# 7:00 PM UTC December 31 = 3:00 AM PHT January 1 - Replenish leave balances
0 19 31 12 * /opt/cron-scripts/yearly-replenish-leaves.sh

# YEARLY TASKS (January 2nd UTC = January 2nd PHT)
# 6:00 PM UTC January 1 = 2:00 AM PHT January 2 - Archive old payrolls
0 18 1 1 * /opt/cron-scripts/yearly-archive-payrolls.sh

# DISK MAINTENANCE (UTC times)
# 9:00 PM UTC = 5:00 AM PHT (+1 day) - Truncate large log files
0 21 * * * /opt/cron-scripts/truncate-large-logs.sh

# 10:00 PM UTC = 6:00 AM PHT (+1 day) - Check disk usage, auto-cleanup at 90%+
0 22 * * * /opt/cron-scripts/disk-usage-check.sh

# 7:00 PM UTC Saturday = 3:00 AM PHT Sunday - Docker system prune
0 19 * * 6 /opt/cron-scripts/docker-system-prune.sh

# 7:30 PM UTC Saturday = 3:30 AM PHT Sunday - Cleanup unused profile images
30 19 * * 6 /opt/cron-scripts/cleanup-unused-images.sh

# 7:00 PM UTC 1st of month = 3:00 AM PHT 2nd of month - Delete old log files (90+ days)
