...
class ServerMaintenanceView(APIView):
    ...
    TRIGGERABLE_COMMANDS = [
        ...
        {"id": "fix_service_payment_sync", "label": "Fix Service Payment Sync", "description": "Reconcile service payment mirrors between main/sub transactions and rebuild out-of-sync sub-stall items", "app": "services", "args": ["--all"], "category": "sales", "destructive": False},
            ...
            if action_type == "install_cron_jobs":
                ...
                try:
                    ...
                    deploy_script = (
                        ...
                        '  sed "/^# RVDC Attendance & Payroll Management Cron Jobs/,/^# END RVDC Cron Jobs/d" "$CRONTAB_FILE" > /tmp/clean.txt 2>/dev/null || cp "$CRONTAB_FILE" /tmp/clean.txt\n'
                        '  grep -v "/opt/cron-scripts/" /tmp/clean.txt > /tmp/clean2.txt 2>/dev/null || cp /tmp/clean.txt /tmp/clean2.txt\n'
                        '  grep -v "^CRON_TZ=Asia/Manila$" /tmp/clean2.txt > /tmp/clean3.txt 2>/dev/null || cp /tmp/clean2.txt /tmp/clean3.txt\n'
                        '  grep -v "^TZ=Asia/Manila$" /tmp/clean3.txt > /tmp/clean.txt 2>/dev/null || cp /tmp/clean3.txt /tmp/clean.txt\n'
                        'else\n'
                        '  touch /tmp/clean.txt\n'
                        'fi\n'
    ...
    def _generate_cron_scripts(container_name):
        ...
        scripts = {
            ...
            "clock-in-reminder.sh": _mgmt(
                "cron-clock-in-reminder.log", "Clock-In Reminder", "send_attendance_reminders",
                "--mode clock_in",
            ),
            "clock-out-reminder.sh": _mgmt(
    ...
    def _generate_crontab_entries():
        """Generate RVDC crontab entries for root crontab (all times in UTC)."""
        return (
            '# ============================================================================\n'
            '# RVDC Attendance & Payroll Management Cron Jobs\n'
            '# Auto-installed via Server Maintenance Dashboard\n'
            '# All times are in UTC (Philippines Time is UTC+8)\n'
            '# ============================================================================\n\n'
            'TZ=Asia/Manila\n\n'
            '# DAILY TASKS (UTC times - logs will show PHT via TZ=Asia/Manila)\n'
            '# 1:00 PM UTC = 9:00 PM PHT - Auto-close open attendance records\n'
            '0 13 * * * /opt/cron-scripts/auto-close-attendance.sh\n\n'
            '# 11:00 PM UTC = 7:00 AM PHT (+1 day) and 11:30 PM UTC = 7:30 AM PHT (+1 day) - Clock-in reminders\n'
            '0,30 23 * * * /opt/cron-scripts/clock-in-reminder.sh\n\n'
            '# 5:00 AM UTC = 1:00 PM PHT and 5:30 AM UTC = 1:30 PM PHT - Half-day clock-out reminders\n'
            '0,30 5 * * * /opt/cron-scripts/clock-out-reminder.sh\n\n'
            '# 10:00 AM UTC = 6:00 PM PHT and 10:30 AM UTC = 6:30 PM PHT - Full-day clock-out reminders\n'
            '0,30 10 * * * /opt/cron-scripts/clock-out-reminder.sh\n\n'
            '# 10:00 PM UTC = 6:00 AM PHT (+1 day) - Notify admins about incomplete attendance before payroll\n'
            '0 22 * * 4 /opt/cron-scripts/weekly-attendance-admin-reminder.sh\n\n'
            '# 3:30 PM UTC = 11:30 PM PHT - Mark absent employees\n'
            '30 15 * * * /opt/cron-scripts/mark-absences.sh\n\n'
            '# 6:00 PM UTC = 2:00 AM PHT (+1 day) - Delete old notifications\n'
            '0 18 * * * /opt/cron-scripts/delete-old-notifications.sh\n\n'
            '# 7:00 PM UTC = 3:00 AM PHT (+1 day) - Cleanup archived quotations\n'
            '0 19 * * * /opt/cron-scripts/cleanup-archived-quotations.sh\n\n'
            '# 9:00 PM UTC = 5:00 AM PHT (+1 day) - Truncate large log files\n'
            '0 21 * * * /opt/cron-scripts/truncate-large-logs.sh\n\n'
            '# 10:00 PM UTC = 6:00 AM PHT (+1 day) - Disk usage check\n'
            '0 22 * * * /opt/cron-scripts/disk-usage-check.sh\n\n'
            '# WEEKLY TASKS\n'
            '# 3:00 PM UTC Fri = 11:00 PM PHT Fri - Fix attendance + refresh payroll\n'
            '0 15 * * 5 /opt/cron-scripts/weekly-attendance-payroll-fix.sh\n\n'
            '# 7:00 PM UTC Sat = 3:00 AM PHT Sun - Docker system prune\n'
            '0 19 * * 6 /opt/cron-scripts/docker-system-prune.sh\n\n'
            '# 7:30 PM UTC Sat = 3:30 AM PHT Sun - Cleanup unused profile images\n'
            '30 19 * * 6 /opt/cron-scripts/cleanup-unused-images.sh\n\n'
            '# YEARLY TASKS\n'
            '# 6:00 PM UTC Dec 31 = 2:00 AM PHT Jan 1 - Update holiday years\n'
            '0 18 31 12 * /opt/cron-scripts/yearly-update-holidays.sh\n\n'
            '# 7:00 PM UTC Dec 31 = 3:00 AM PHT Jan 1 - Replenish leave balances\n'
            '0 19 31 12 * /opt/cron-scripts/yearly-replenish-leaves.sh\n\n'
            '# 6:00 PM UTC Jan 1 = 2:00 AM PHT Jan 2 - Archive old payrolls\n'
            '0 18 1 1 * /opt/cron-scripts/yearly-archive-payrolls.sh\n\n'
            '# MONTHLY - 7:00 PM UTC 1st = 3:00 AM PHT 2nd - Delete old log files (90+ days)\n'
            '0 19 1 * * find /var/log/cron-*.log -type f -mtime +90 -delete\n\n'
            '# ============================================================================\n'
            '# END RVDC Cron Jobs\n'
            '# ============================================================================\n'
        )
